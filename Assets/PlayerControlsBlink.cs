...
public class PlayerControlsBlink : MonoBehaviour
{
    public bool cannotCling;
    ...
    private RaycastHit2D blinkRayCast;
    ...
    void Update()
    {
        ...
        if (Time.timeScale == 1)
        {
            ...
            if (clinging == false)
            {
                Debug.DrawLine(new Vector2(this.transform.position.x + (.25f * direction), this.transform.position.y), new Vector2(this.transform.position.x + (3.3f * direction), this.transform.position.y), Color.green);
                blinkRayCast = Physics2D.Linecast(new Vector2(this.transform.position.x + (.25f * direction), this.transform.position.y), new Vector2(this.transform.position.x + (3.3f * direction), this.transform.position.y));

                if (blinkRayCast.transform != null)
                {
                    if (blinkRayCast.transform.name != "NoBlink")
                    {
                        if (Input.GetButtonDown("Blink") && blinkCoolDown < 0)
                        {
                            blinkFade = true;
                            blinkCoolDown = .75f;
            ...
            {
                // Ready to slide
                if (readyToSlide == true && Time.time >= slideTimer + .8f)
                {
    ...
    private void BlinkActivater()
    {
        if (blinkFade == true)
        {
            sprite.color = new Color(sprite.color.r, sprite.color.g, sprite.color.b, alpha);
            alpha -= .15f;
            //Can fall through floors consider changing
            boxCol[0].enabled = false;
            boxCol[1].enabled = false;

        }
        if (alpha <= 0 && blinkFade == true)
        {
            this.transform.position = new Vector2(this.transform.position.x + (2 * blinkDirection), this.transform.position.y);
            boxCol[0].enabled = true;
            boxCol[1].enabled = true;
            blinkFade = false;
        }
        if (alpha <= 1 && blinkFade == false)
        {
            alpha += .15f;
            sprite.color = new Color(sprite.color.r, sprite.color.g, sprite.color.b, alpha);
            if (alpha >= 1)
